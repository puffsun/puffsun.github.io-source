...
Promise 对象表示一个目前尚不可用，但将来某个时间点可以被获得的值。一个浅显易懂的比方是，比如你到一个店了买一杯咖啡，收银员收钱以后给了你一张收费凭据，那么虽然你没有马上得到这杯咖啡，但在将来你可以凭借这个收费凭据得到它。

在你等待这杯咖啡制作的过程中，你可以发微信给朋友，告诉她：我点了咖啡，你过来和我一起喝一杯吧。现在可能会出现两种结果：

1. 几分钟之后，你的咖啡冲好了，你可以用收费凭证换取这杯咖啡，然后你就和朋友愉快的享受这杯咖啡了；
...
得到了 `promise` 对象以后，我们通过它暴露的 `then` 接口来使用它。`then` 接受两个参数，第一个是成功的回调函数（被 `resolve` 后调用），第二个是失败的回调函数（被 `reject` 后调用）。
